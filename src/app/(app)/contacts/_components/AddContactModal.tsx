      ...
      <div className="bg-bg/95 backdrop-blur-md border border-glass-border rounded-[2.5rem] shadow-2xl max-w-md w-full p-8 relative animate-in zoom-in-95 duration-300">
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-5 mb-8 text-left">
            ...
            <div>
              <label className="block text-[10px] font-black text-fg/30 uppercase tracking-widest mb-2 px-1">Phone Number</label>
              <div className="flex gap-2">
                <select
                  value={countryCode}
                  onChange={(e) => setCountryCode(e.target.value)}
                  className="bg-glass-input border border-glass-border rounded-2xl px-3 py-4 text-xs font-bold text-fg focus:border-indigo-500 focus:outline-none shrink-0 cursor-pointer"
                >
                  {COUNTRY_CODES.map((c) => (
